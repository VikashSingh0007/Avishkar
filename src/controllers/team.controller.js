...
const deleteTeam = async (req, res, next) => {
  ...
  try {
    ...
    if (!team || !user) {
      ...
      res.json({
        error: "not found",
        message: "team or user was not found!",
        success: false,
      });
      ...
      res.json({
        error: "unauthorized",
        message: "not authorised! only leader can delete a team!",
        success: false,
      });
    } else {
      ...
      team.pendingMembers.forEach(async (memberId) => {
        ...
        teamMember.pendingTeam = teamMember.pendingTeam.filter(
          (pendingTeamId) => {
      ...
      team.acceptedMembers.forEach(async (memberId) => {
        const teamMember = await User.findOne({ _id: memberId });
        teamMember.participatingTeam = teamMember.participatingTeam.filter(
          (pendingTeamId) => {
            return pendingTeamId != teamId;
          }
        );
...
const updateTeam = async (req, res, next) => {
  ...
  const id = req.user._id;
  if (!teamId || !teamName) {
    //check if teamid or name is missing
    res.statusCode = 400;
    res.json({
  ...
  try {
    const team = await Team.findOne({
      //find the team whose name need's to be updated
      _id: teamId,
    });
    ...
    if (!team) {
      ...
      // case when team Name is already taken
      res.statusCode = 400;
      res.json({
      ...
      res.json({
        error: "unauthorized",
        message: "not authorised! only leader can update a team!",
        success: false,
      });
    } else {
...
const sendTeamInvitation = async (req, res, next) => {
  ...
  try {
    ...
    if (!team || !targetMember) {
      ...
      res.json({
        error: "unauthorized",
        message: "not authorised! only leader can send invite to a player!",
        success: false,
      });
    } else {
      const sendableInvites =
        team.size - team.pendingMembers.length - team.acceptedMembers.length;
...
const acceptOrRejectInvite = async (req, res, next) => {
  ...
  const id = req.user._id;
  if (!teamId || status == null) {
    // in case id or status are incorrectly given
    res.statusCode = 400;
    res.json({
  ...
  try {
    ...
    if (!teamuser) {
      // if the user to be invited does not exist
      res.statusCode = 400;
      res.json({
    ...
    if (!teamuser.isFeePaid) {
      //if there is a fee payment issue
      res.statusCode = 400;
      res.json({
        error: "fee payment issue",
        message:
          "cannot participate or invite team until participation fee is paid!",
        success: false,
      });
      return;
    } else {
      ...
      if (!team) {
        // case where the team to be invited does not exist
        res.statusCode = 404;
        res.json({
          error: "not found",
          message: "team or invite not found!",
          success: false,
        });
        return;
      } else {
        if (status) {
          ...
          teamuser.pendingTeam = teamuser.pendingTeam.filter(
            (participateTeamId) => {
          ...
          teamuser.participatingTeam = addUserToDepartment(
            teamuser.participatingTeam,
            team._id
          );

          // add member to team's accepted
          team.acceptedMembers = addUserToDepartment(
            team.acceptedMembers,
            teamuser._id
          );
...
const getAllTeamParticipating = async (req, res, next) => {
  ...
  try {
    ...
    const teams = await tUser.populate({
      path: "participatingTeam",
      populate: ["acceptedMembers", "pendingMembers"],
    });
  ...
  try {
    ...
    if (!team) {
      ...
      if (team.acceptedMembers) {
        acceptedMembers = await Promise.all(
          team.acceptedMembers.map(async (memberId) => {
      ...
      if (team.pendingMembers) {
        pendingMembers = await Promise.all(
          team.pendingMembers.map(async (memberId) => {
            const acceptedUser = await User.findOne({ _id: memberId });
            return {
              name: acceptedUser.name,
              email: acceptedUser.email,
