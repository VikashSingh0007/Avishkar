...
module.exports = {
    generateVerificationToken:async function(user){
            const token=await jwt.sign(user,process.env.VERIFICATION_TOKEN_SECRET,{expiresIn:'59m'})
    ...
    verifyToken:async function(token){
        try {
            ...
            const payload= jwt.verify(jwttoken,process.env.VERIFICATION_TOKEN_SECRET);
            return {
                message:payload,
                flag:true
            }
