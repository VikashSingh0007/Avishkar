...
const mailgun = require("mailgun-js");
 ...
 async function sendUserVerificationMail(email,token) {
  ...
  async function sendUserVerificationMailOtherCollege(email,token) {
    const mailSubject = "User Email Verification 👩‍💻";
    const mailMessage = `<!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <div class="container">
      ...
      <h2 style="text-align: center;">
        Your Account Will Be Activated After Fee Payment Verification
      </h2>
      </div>
    </body>
    </html>
  ...
  async function sendActivationNotice(email,status){
    const mailSubject = "Account Activation Notice";
    const mailMessage = `<!DOCTYPE html>
    <html lang="en">
    <head>
    <meta charset="UTF-8">
 ...
 async function sendUserResetPassword(email, token) {
  ...
  // }
   ...
   async function mailSender(email, subject, message) { //mail gun 
      try {
        ...
        var mailOptions = {
            from : "Culrav-Avishkar <admin@culrav-avishkar.com>",
            to : email,
            subject : subject,
            html : message,
        }
